...
QPixmap DiagramaWidget::crearImagenDiagrama(MBR mbr)
{
    ...
    // DIVIDIR EN 4 SECCIONES IGUALES (los extremos)
    int anchoSeccion = anchoDisponible / 15; // Cada sección extrema = 1/20 del total

    // ===== CUADRO 1: MBR (1 unidad) =====
    int anchoMBR = anchoSeccion;
    painter.drawRect(x, y, anchoMBR, altura);
    painter.drawText(QRect(x, y, anchoMBR, altura), Qt::AlignCenter, "MBR");
    x += anchoMBR;

    // ===== CUADRO 2: LIBRE IZQUIERDO (1 unidad) =====
    int anchoLibreIzq = anchoSeccion;
    painter.drawRect(x, y, anchoLibreIzq, altura);
    painter.drawText(QRect(x, y, anchoLibreIzq, altura), Qt::AlignCenter, "Libre");
    x += anchoLibreIzq;

    // ===== CUADRO 3: EXTENDIDA (16 unidades - el espacio del medio) =====
    ...
    painter.drawText(QRect(x, y + 5, anchoExtendida, 20), Qt::AlignCenter, "Extendida");

    // Línea divisoria debajo de "Extendida"
    painter.drawLine(x, y + 28, x + anchoExtendida, y + 28);

    // DIVIDIR LA EXTENDIDA EN 5 PARTES
    int xLogica = x + 2;
    int yLogica = y + 30;
    int alturaLogica = altura - 32;
    int anchoLogica = (anchoExtendida - 4) / 5; // Dividir en 5

    // EBR 1
    painter.drawRect(xLogica, yLogica, anchoLogica, alturaLogica);
    painter.drawText(QRect(xLogica, yLogica, anchoLogica, alturaLogica), Qt::AlignCenter, "EBR");
    xLogica += anchoLogica;

    // LÓGICA 1
    painter.drawRect(xLogica, yLogica, anchoLogica, alturaLogica);
    painter.drawText(QRect(xLogica, yLogica, anchoLogica, alturaLogica), Qt::AlignCenter, "Lógica");
    xLogica += anchoLogica;
    ...
    // ===== CUADRO 4: PRIMARIA (1 unidad) =====
    int anchoPrimaria = anchoSeccion;
    painter.drawRect(x, y, anchoPrimaria, altura);
    painter.drawText(QRect(x, y, anchoPrimaria, altura), Qt::AlignCenter, "Primaria");
    x += anchoPrimaria;

    // ===== CUADRO 5: LIBRE DERECHO (1 unidad) =====
    int anchoLibreDer = anchoSeccion;
    painter.drawRect(x, y, anchoLibreDer, altura);
    painter.drawText(QRect(x, y, anchoLibreDer, altura), Qt::AlignCenter, "Libre");
